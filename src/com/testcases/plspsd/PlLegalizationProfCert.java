...
public class PlLegalizationProfCert extends TestBase {
	
	String testname = "PlLegalizationProfCert";
...
//		tr1.specialInspectionPlumbingNew(data.get("tr1"));
		tr1.specialInspectionPlumbingLegalization(data.get("tr1"));
...
/*	@Test(priority = 0, dataProvider = "getTestData")
	public void Base(Hashtable<String, String> data) {
		if (!TestUtil.isExecutable("PlLegalization", xlsx) || data.get("Runmode").equals("N"))
			throw new SkipException("Skipping test");
		System.out.println("BEGIN " + convertedTimestamp() + " **************** " + data.get("description"));
		test = rep.startTest(data.get("description"));
		test.log(LogStatus.INFO, data.get("description"));
